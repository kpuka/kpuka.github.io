...
To use R, launch RStudio. RStudio provides a user-friendly interface for working with R. Upon opening RStudio you'll be greeted with a window divided into multiple panes, including the script editor, console, environment, and files.

The **script editor** is where you write and edit your R code. The **console** is where you see the output of your commands and interact with R directly. The **environment pane** displays information about the variables and objects in your current R session, while the **files pane** allows you to navigate your computer's file system and manage your R projects.

To execute code in RStudio, highlight the portion of code you would like to execute and presee "Run" (located in the top right corner of the script editor. On Windows, you can alsu use the shortcut "Ctrl + Enter".

To follow along with this tutorial, the code and syntax written below should be written in the **script editor**. 
...
# set.seed(111)
# nhis2018 <- read.csv("data_intro_to_r/nhis_2018_SampleAdult_complete.csv") %>%
#   sample_n(500) %>%
#   mutate(ID= 1:500) %>%
...
# 
# write.csv(nhis2018, "data_intro_to_r/nhis2018_sampleadult.csv", row.names = FALSE)
...
-   We will use a subset of the data from the "Sample Adult Interview":
    -   To download this data, <a href="files/intro_to_r/nhis2018_SampleAdult.csv" target="_blank">click here</a>
    -   To download the codebook, <a href="files/intro_to_r/nhis_codebook.csv" target="_blank">click here</a> 
...
library(knitr)
library(kableExtra)
read.csv("files/intro_to_r/nhis2018_SampleAdult.csv") |> 
...
read.csv("files/intro_to_r/nhis_codebook.csv") |> 
...
-   To work with data, you first need to read it into R
-   `read.csv()` opens CSV files; specify the file location/name

```{r}
#| warning: false

nhis2018 <- read.csv("files/intro_to_r/nhis2018_SampleAdult.csv")

  # In this example, the folder that contains the 'R Project' file, 
  # also contains a folder called 'files, within which is a folder 
  # called "intro_to_R" which has the CSV data filed called 
  # 'nhis2018_SampleAdult.csv'

  # Use "<-" to save the data and name it 'nhis2018'

```

To load other file types:
...
```{r}
#| eval: false
saveRDS(nhis2018, "files/intro_to_r/updated_nhis2018.R")
...
```{r}
#| eval: false
write.csv(nhis2018, "files/intro_to_r/updated_nhis2018.csv")
```
...
Create a project-specific folder that contains the following:

-   '**R Project**' file to better link data and scripts
...
# Load data
nhis2018 <- read.csv("data_intro_to_r/nhis2018_SampleAdult.csv") |>
  clean_names() |> 
  remove_empty ("cols")
...
saveRDS(nhis2018, "data_intro_to_r/nhis2018_clean.R")
...
# Load data
data_clean <- readRDS("data_intro_to_r/nhis2018_clean.R")
