...
To get started, open RStudio and go to: File -> New File -> Quarto Document. 
![](files/intro_to_quarto/open_quarto.png)

<br>
In RStudio, you can click "Render" to generate the final document. You can also highlight code and click "Run" to execute specific lines of code. You can also view the file using the 'Source' view, which uses the raw syntax (used throughout this tutorial), or use the 'Visual' view, which uses a point and click interface for formatting. 

![](files/intro_to_quarto/open_quarto2.png)
...
To create and work with columns, use `:::`. Like brackets, these are used to indicate the start and end the content to be placed in columns. 
...
Sections can also be collapsed using HTML code, as shown below in @sec-html_folding.
...
![](files/intro_to_quarto/Bootswatch example.jpg)
...
![](files/intro_to_quarto/newfile.png)
