...
-   R: `nhis2018 <- readRDS("data_name.rds")`
...
```{r}
#| eval: false
saveRDS(nhis2018, "updated_nhis2018.rds")
...
Use `filter_out()` to specify the *rows* (aka observations) to **_remove_**
...
|`filter_out(marital = 3)`               | marital does not equal 3 | 
|`filter(!is.na(marital))`               | marital is not missing | 
|`filter_out(is.na(marital))`            | marital is not missing | 
...
Use `recode_values()` to manually recode any variable, to assign old values to new values

-   **_Important_**: The order of statement matters for `recode_values()` - the new variable will have the value associated with the first condition that is met. To avoid unexpected results, ensure that your conditions (or value mappings) are mutually exclusive and not duplicated.




::: columns
::: {.column width="60%; font-size: 90%;"}
```{r}
#| output: false
...
nhis2018 <- nhis2018 |>
 mutate(
   ...
   # Create new categorical variable from a categorical variable variable
   bmi_binary = recode_values(bmi_category, 
   ...
   # Create categorical variable from a continuous variable
    smoking_label = recode_values(smoking,
                      # old_values ~ new_values
...
# Check
nhis2018 |>
  select (id, bmi, bmi_category, bmi_binary, smoking, smoking_label) |>
  head(13) # The number indicates how many rows/observations to print
...
**1. Misspelling object or variable names**

R is case-sensitive, so `Age`, `age`, and `AGE` are different objects.

`mean(data$Age)` is not the same as `mean(data$age)`

If you see `object 'age' not found` check spelling and capitalization carefully.

---

**2. Forgetting quotation marks around text**

Text values should be enclosed in quotation marks.

Use `filter(data, sex == "Female")` rather than `filter(data, sex == Female)`

---

**3. Using `=` instead of `==` when making comparisons**

Use `sex == "Female"` to test whether something is equal to a value.

Use `new_variable = valuw` when assigning a value.

---

**4. Forgetting to load required packages**
...
you may need to load the package first: 

```r
library(dplyr)
```

Packages generally only need to be installed once, but they must be loaded each time you start a new R session.

---

**5. Running code line-by-line without running previous code**

Many errors occur because an object has not yet been created.
...
before creating `clean_data`, R will return an error.

When working through scripts, ensure that earlier sections have been run first.

---

**6. Working in the wrong project or folder**

Many file import errors occur because R is looking in a different folder than expected.

Using R Projects (as described above) helps prevent these issues by keeping all files and scripts organized within a single working directory.


### Troubleshooting Steps

If code that previously worked suddenly stops working, try the following:

#### Step 1: Restart R

Sometimes objects from previous sessions remain in memory and can interfere with your current work.

The easiest solution is often to completely restart R: close and reopen RStudio.

This ensures you are starting with a clean environment.

---

#### Step 2: Reinstall or Update Packages

If package-related errors occur, make sure you have the latest versions installed.
...
After installation, load the package again:
...
Older versions of R can sometimes cause compatibility issues with newer packages.
...
After updating R, you may need to reinstall or update your packages.
...
usually means that an object has not been created or has been misspelled.

```r
could not find function "filter"
```

usually means that a package has not been loaded.

Reading the full error message is often the fastest path to a solution.

If you're still stuck, copy the exact error message and search online, such as through Google or ChatGPT.
